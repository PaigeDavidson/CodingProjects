...
public class AVLTree<AnyType extends Comparable<? super AnyType>>
{
    ...
    {
        //checks if there is no tree
        if(t == null){
            return null;
        }
        // if you are at the bottom of the tree, return the right child
        // sometimes the right child will be null
        // this is where the delete happens because the node will be replaced either with the value of the right child or null if there is no right child
        if(t.left == null){
            return t.right;
        }
        // if t has a left child, travel recursively down the left side
        // you assign t.left to the return value of the function which is the new tree after deletion
        //its returning a node which is the root of the  new subtree which could have changed - it just returned it to be safe
        t.left = deleteMin(t.left);
        // now you have to balance the tree and return the whole tree instead of just the left subtree
        return balance(t);

        //instead of finding the smallest number find the leftmost node and delete the leftmost node
        //remember to balance the tree
    }
    /**
